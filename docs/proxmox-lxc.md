...
`--ctid` is optional connection metadata for a local native solver; omit it for an external endpoint. `set-solver` checks a nonempty endpoint, backs up before changing managed configuration and preserves unrelated settings and secrets. It updates the app, novel engine and Suwayomi, then restarts them and checks readiness; manual configuration edits are unnecessary. A recorded CT ID does not follow DHCP address changes automatically: use a stable address or rerun `--reconnect`.

When upgrading from a version whose installed manager only configured the manga consumers, run `set-solver` or `--reconnect` once after the first app update. This populates the existing novel environment without regenerating its token. Updates run by the new manager also migrate the saved solver URL into `novel.env`.
...
The novel engine executes approved LNReader scripts in QuickJS with guarded networking and bounded timers. All sources start disabled. Its registry contains 278 published scripts; 270 pass the static host-capability scan, which does not establish live site availability. Royal Road and public guest-readable AO3 works were live-tested in the initial application verification. When configured, FlareSolverr gets clearance for detected browser challenges before one guarded retry of the original request, preserving actual HTTP status and JSON bodies. Explicit browser fetches support rendered GET and form POST. Consent/login pages, unsolved challenges and unsupported browser capabilities return explicit errors. Binary images retain direct guarded transport with matching solver cookies. The solver is a trusted browser egress boundary: the engine validates initial/final destinations, but cannot pin its DNS or inspect intermediate browser redirects/subresources. See [runtime compatibility, network limits and source updates](../novel-engine/README.md).
