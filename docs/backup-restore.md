...
Restore into a **new empty** installation; the command refuses a nonempty database or destination volume. Use the same checkout revision and PostgreSQL major version initially. The published images must be pulled, or the development images built, before restore.
...
2. Run `bash scripts/miaoyomi-setup.sh --config-only` to prepare empty bind directories, then `docker compose pull` to fetch the published app and runtime. For a source checkout instead, run `MIAOYOMI_COMPOSE_MODE=dev bash scripts/miaoyomi-setup.sh --config-only` followed by `docker compose -f docker-compose.yml -f docker-compose.dev.yml build`. Keep writers stopped.
3. Restore and start:
...
For a source checkout, prefix the restore with `MIAOYOMI_COMPOSE_MODE=dev` and start it with `docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d --build --wait`.
