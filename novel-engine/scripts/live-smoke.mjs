...
for (const id of process.argv.slice(2).length ? process.argv.slice(2) : ['royalroad', 'archiveofourown', 'scribblehub']) {
  ...
  const call = (method, args) => executePlugin(script, method, args, { deadlineMs: 30_000, fetch: (url, init, signal) => broker.fetch(entry, url, init, signal) });
  ...
  try {
    ...
    report.ok = true;
  } catch (error) { report.ok = false; report.error = error.code || 'SMOKE_FAILURE'; report.message = error.message; if (id !== 'scribblehub' || error.code !== 'SITE_CHALLENGE') process.exitCode = 1; }
